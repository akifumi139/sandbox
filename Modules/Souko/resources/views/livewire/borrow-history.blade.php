        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                <flux:select wire:model.live="status" label="状態">
                    ...
                    <flux:select.option value="active">貸出中</flux:select.option>
                    <flux:select.option value="returned">返却済み</flux:select.option>
                </flux:select>
            </div>
        </div>
    </flux:card>
    ...
    <flux:card class="p-0 overflow-hidden">
        <div class="p-4 sm:px-6 border-b border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
            <div>
                ...
                <flux:subheading>{{ $this->logs->total() }}件の貸出</flux:subheading>
            </div>
        ...
        <flux:table class="px-3">
            <flux:table.columns>
                ...
                <flux:table.column>返却日時</flux:table.column>
                <flux:table.column>状態</flux:table.column>
            </flux:table.columns>

            <flux:table.rows>
                @forelse ($this->logs as $log)
                    <flux:table.row wire:key="lending-{{ $log->id }}">
                        <flux:table.cell>
                        ...
                        <flux:table.cell variant="dim" class="whitespace-nowrap">
                            {{ $log->return_at?->format('Y/m/d H:i') ?? '-' }}
                        </flux:table.cell>
                        <flux:table.cell>
                            <flux:badge :color="$log->return_at === null ? 'amber' : 'green'" size="sm" inset="top bottom">
                                {{ $log->return_at === null ? '貸出中' : '返却済み' }}
                            </flux:badge>
                        </flux:table.cell>
                    </flux:table.row>
                @empty
