...
it('creates a borrow log and marks the tool as rented when checkout is completed', function (): void {
    ...
    expect($tool->status)->toBe('rented')
        ->and(ToolLog::query()->where('tool_id', $tool->id)->where('action_type', 'borrow')->count())->toBe(1)
        ->and(ToolLog::query()->where('tool_id', $tool->id)->where('action_type', 'borrow')->first()->user_name)->toBe('山田 太郎');
});
